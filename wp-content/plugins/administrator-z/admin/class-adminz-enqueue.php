...
class ADMINZ_Enqueue extends Adminz {
	...
	function __construct() {
		...
		add_action( 'wp_head', [$this,'enqueue_custom_font'],999);
		add_action( 'wp_head', [$this,'enqueue_custom_css'],999); // vi la custom text nen phai dat hook la wp_head 999
		add_action( 'wp_enqueue_scripts', [$this,'enqueue_supported_font'],999);
		add_action( 'wp_enqueue_scripts', [$this,'enqueue_uploaded_js'],999);
		add_action( 'wp_enqueue_scripts', [$this,'adminz_enqueue_scripts'],999);
		add_action( 'wp_enqueue_scripts', [$this,'adminz_enqueue_styles'],999);
		add_action( 'wp_footer', [$this,'adminz_enqueue_custom_scripts'],999);
	...
	function get_fonts(){
		...
		if(!empty($font_files) and is_array($font_files)){
			?>			
			<textarea style="display: none; " cols="100" rows="10" name="adminz_enqueue[adminz_fonts_uploaded]"><?php echo $this->get_option_value('adminz_fonts_uploaded'); ?></textarea>
	...
	function get_css(){
		...
		if(!empty($css_files) and is_array($css_files)){
			?>
			<textarea style="display: none;" cols="100" rows="10" name="adminz_enqueue[adminz_css_uploaded]"><?php echo $this->get_option_value('adminz_css_uploaded'); ?></textarea>
			<div style="padding: 10px; background: white;">            						
				<table>
					<tr>
		...
		<form method="post" action="options.php">
	        ...
	        <table class="form-table">
				...
				<tr valign="top">
	                ...
	                <td> 						
	                	...
	                	<label>
	                		...
	                		?>
	                		<input <?php echo $checked; ?> type="checkbox" value="lato" name="adminz_enqueue[adminz_supported_font][]" /> Lato vietnamese
	                	</label><br>
	                	<label>
	                		<?php 
	                		$checked = in_array("fontawesome",$adminz_supported_font) ? "checked" : "";
	                		?>
	                		<input <?php echo $checked; ?> type="checkbox" name="adminz_enqueue[adminz_supported_font][]" value="fontawesome"/> Font Awesome 
	                	...
	                	<label>
	                		...
	                		?>
	                		<input <?php echo $checked; ?> type="checkbox" name="adminz_enqueue[adminz_supported_font][]" value="icofont"/> Icofont 
	                	...
	                	<label>
	                		...
	                		?>
	                		<input <?php echo $checked; ?> type="checkbox" name="adminz_enqueue[adminz_supported_font][]" value="eicons"/> Eicons 
	            ...
	            <tr valign="top">
	            	<th>Custom CSS</th>
	            	<td>
	            		<textarea class="adminz_css_editor" style="width: 100%; background: #f2f2f2; border: 3px solid gray;" rows="10" name="adminz_enqueue[adminz_custom_css_fonts]" placeholder="Your custom css here will be enqueue in header..."><?php echo $this->get_option_value('adminz_custom_css_fonts'); ?></textarea>
	            ...
	            <tr valign="top">
	            	...
	            	<td>
	            		<textarea class="adminz_js_editor" style="width: 100%; background: #f2f2f2; border: 3px solid gray;" rows="10" name="adminz_enqueue[adminz_custom_js]" placeholder="Code inside script tag will be enqueue in footer..."><?php echo $this->get_option_value('adminz_custom_js'); ?></textarea>
	            		<?php echo submit_button(); ?>
	            	</td>
	            </tr>
				<tr valign="top">
					...
					<td>
						<?php 	                	
	                	$adminz_supported_js = $this->get_option_value('adminz_supported_js',false,[]);
	                	?>
						<label>
	                		<input type="checkbox" name="adminz_enqueue[adminz_supported_js][]" value="flickity" <?php if(in_array('flickity',$adminz_supported_js)) echo "checked"; ?>> Flickity
	                	</label><br>
	                	<label>
	                		<input type="checkbox" name="adminz_enqueue[adminz_supported_js][]" value="fotorama" <?php if(in_array('fotorama',$adminz_supported_js)) echo "checked"; ?>> Fotorama
	                	</label><br>
					</td>
				</tr>
				<tr valign="top">
	                ...
	                <td>
						...
						foreach ($GLOBALS['wp_scripts']->registered as $handle=> $obj){							
							...
							echo '<label><input class="adminz_enqueue_registed_js_" type="checkbox" name="adminz_enqueue[adminz_enqueue_registed_js_][]" value="'.$handle.'" '.$checked.' /> '.$handle.'<code>'.$link.'</code></label><button class="show_js_data" type="button" style="border: none; cursor: pointer;">...</button></br>';
				...
				<tr valign="top">
					...
					<td>
						...
						foreach ($GLOBALS['wp_styles']->registered as $handle => $obj) {							
							...
							echo '<code>'; print_r($obj->translations_path); echo '</code>'; 
							$objhtml = ob_get_clean();
							echo '<label><input class="adminz_enqueue_registed_css_" type="checkbox" name="adminz_enqueue[adminz_enqueue_registed_css_][]" value="'.$handle.'" '.$checked.' /> '.$handle.'<code>'.$link.'</code></label><button class="show_js_data" type="button" style="border: none; cursor: pointer;">...</button></br>';
							echo "<div class='more_info hidden'><pre>";echo $objhtml; echo "</pre></div>";
						}
						 ?>
					</td>
	...
	function enqueue_custom_font(){		
		$fonts = $this->get_option_value('adminz_fonts_uploaded');
	...
	function enqueue_custom_css(){
		$css = $this->get_option_value('adminz_custom_css_fonts');
		if($css){ 
			ob_start();
			?>
	...
	function adminz_enqueue_styles(){		
 	...
 	function adminz_enqueue_custom_scripts(){ 		
 		$custom_script = $this->get_option_value('adminz_custom_js');
 		if($custom_script){
