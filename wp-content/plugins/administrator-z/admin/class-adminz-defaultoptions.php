...
class ADMINZ_DefaultOptions extends Adminz {
	...
	function __construct() {
		...
		if(is_admin()){
			if($this->check_option('auto_image_excerpt',false,"on")){
				add_action( 'add_attachment', [$this,'my_set_image_meta_upon_image_upload'] );	
			}
		}			
	}
	...
	function general_admin_notice(){		
		...
		echo '<p>';
		echo '<strong>'.$this->get_adminz_menu_title().' notice:</strong> '. $this->get_option_value('adminz_notice');
	...
	function wp_configs(){
		...
		if($this->check_option('adminz_use_category_tiny_mce',false,"on")){		
			add_filter('category_edit_form_fields', function ($tag) {
			    ?>
	...
	function init(){
		$adminz_autoupdate = $this->get_option_value('adminz_autoupdate');		
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
	            ...
	            <tr valign="top">
	                ...
	                <td>
 						<input type="text" name="adminz_default[adminz_menu_title]" value="<?php echo $this->get_option_value('adminz_menu_title'); ?>" />
	                </td>
	            </tr>
	            <tr valign="top">
	                ...
	                <td>
 						...
 						if( $image = wp_get_attachment_image_src( $image_id ) ) {
							...
							echo '<a href="#" class="button adminz-upl"><img src="' . $image[0] . '" /></a>
							      ...
							      <input type="hidden" name="adminz_default[adminz_login_logo]" value="' . $image_id . '">';
	            ...
	            <tr valign="top">
	            	...
	            	<td>
 						<input type="text" name="adminz_default[adminz_logo_url]" value="<?php echo $this->get_option_value('adminz_logo_url'); ?>" />
	                </td>
	            </tr>
	            <tr>
					<th scope="row">
				...
				<tr valign="top">
					...
					<td>
						...
						foreach ($update_array as $value) {
							echo '<label> <input type="checkbox" name="adminz_default[adminz_autoupdate][]" value="'.$value[0].'"';
					...
					<td>
						<?php 
							...
							foreach ($GLOBALS[ 'menu' ] as $value) {
								...
								?>
								<label>
							   		<input type="checkbox" name ="adminz_default[adminz_hide_admin_menu][]" value="<?php echo $value[2]; ?>" 
				...
				<tr valign="top">
					...
					<td>
						<textarea placeholder="Leave empty for remove notice" cols="200" rows="5" id="notices" type="textarea" name="adminz_default[adminz_notice]"><?php echo $this->get_option_value('adminz_notice'); ?></textarea>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row">
					...
					<td>
						...
						?>
						<label><input type="checkbox" <?php echo $checked; ?> name="adminz_default[adminz_use_classic_editor]">
						<em>Use classic editor</em></label>
						</br>	
						<?php 
						$checked = "";
						if($this->check_option('adminz_use_category_tiny_mce',false,"on")){
						...
						?>					
						<label><input type="checkbox" <?php echo $checked; ?> name="adminz_default[adminz_use_category_tiny_mce]">
						<em>Category editor Tiny MCE</em></label>
					</td>
				</tr>
	        	...
	        	<tr valign="top">
	        		...
	        		<td>
	        			<div>	   
	        				...
	        				?>     				
	        				<label><input type="checkbox" <?php echo $checked; ?>  name="adminz_default[auto_image_excerpt]"/>Enable to Auto fill image information like image title </label>
	        			</div>
	        		</td>
	    		</tr>
	    		<tr valign="top">
	        		...
	        		<td>
	        			...
	        			foreach (get_taxonomies() as $key => $tax) {
	        				?>
	        				<label>
	        					<input type="checkbox" name="adminz_default[adminz_tax_thumb][]" value="<?php echo $tax; ?>" <?php if(in_array($tax,$tax_thumb)) echo "checked"; ?> />
	        					<?php echo $tax; ?>
	    					</label></br>
	        				<?php
	        			}
				...
				<tr valign="top">
					...
					<td>						
						...
						if($this->check_option('adminz_enable_countview',false,"on")){
					 	...
					 	?>
						<input type="checkbox" name="adminz_default[adminz_enable_countview]" <?php echo $checked; ?>> Enable count view function
					</td>
				</tr>
	        </table>	        
			<?php submit_button(); ?>
