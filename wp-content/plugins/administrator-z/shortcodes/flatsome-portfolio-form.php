...
function adminz_flatsome_portfolios_form_builder(){        
    ...
    add_ux_builder_shortcode('adminz_flatsome_portfolios_form', array(
        'name'      => "Search ". ADMINZ_Helper_Custom_Portfolio::$customname,
...
function adminz_flatsome_portfolios_form_function($atts, $content = null ){        
    ...
    ?>
    <style type="text/css">.adminz_portfolio_form .viewmore .icon-angle-up{display: none;}.adminz_portfolio_form .viewmore.toggled .icon-angle-up{display: inline-block;} .adminz_portfolio_form .viewmore.toggled .icon-angle-down{display: none;} .adminz_portfolio_form i.v-center{margin-top: -.4em}@media (min-width:  550px){.adminz_portfolio_form .field-title{display: flex; flex-direction: row-reverse; } .adminz_portfolio_form .field-title.has-left-title .relative .relative>*{margin-left:  -1px;} } .adminz_portfolio_form i.v-center{margin-top: -.4em} @media (max-width:  549px){.adminz_portfolio_form .absolute.v-center.right{position: static !important; transform:  unset !important; } }</style>
    <form id="<?php echo $_id; ?>" class="adminz_portfolio_form mb-0" method="get" action="<?php echo get_permalink($target);?>">        
        ...
        <?php  
            ...
            $sqlresult = ADMINZ_Helper_Custom_Portfolio::adminz_get_all_meta_key_value('featured_item',$metakey);
                            ...
                            <div class="col-inner">
                                <div class="field-title <?php if($field_before_title){ echo 'has-left-title';} ?>">
                                    <div class="relative full-width">
                                        <div class="relative">
                                            <input type="text" name="search" placeholder="<?php echo $search_placeholder; ?>" style="padding-left:  40px; " value="<?php if(isset($_GET['search'])) echo $_GET['search']; ?>">
...
function admz_portfolio_form_field_metakey($value, $col, $selectnone, $get ,$sqlresult){
    ...
    $metalabel = ADMINZ_Helper_Custom_Portfolio::get_meta_key_label($value);
    if($metalabel){
    ?>
    <div class="<?php echo $col;?>">
        <select name="<?php echo $value; ?>">
            <option value=""><?php echo $selectnone." ".$metalabel ;?> </option>
            <?php 
            if(!empty($list_all_meta_value) and is_array($list_all_meta_value)){
                foreach ($list_all_meta_value as $meta_value) {            
                    if(isset($_GET[$value]) and ($_GET[$value] == $meta_value)){
                        $selected = "selected";
                    }else{
                        $selected = "";
                    }                    
                    echo '<option '.$selected.' value="'.$meta_value.'">'.$meta_value.'</option>';
                }
            }
            ?>
        </select>
...
function admz_portfolio_form_field_tax($value,$col,$selectnone,$get){
    ...
    <div class="<?php echo $col;?>">
        ...
        ?>
        <select name="<?php echo $value; ?>">
            <option value=""><?php echo $selectnone." ".$taxobj->label ;?> </option>
            <?php 
